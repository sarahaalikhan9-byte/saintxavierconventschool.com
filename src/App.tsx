...
export default function App() {
  ...
  const T = {
    bg:         isGlass ? '#FFF7ED'         : '#0A0A0B',
    text:       isGlass ? '#431407'         : '#D1D5DB',
    headerBg:   isGlass ? 'rgba(255,255,255,0.88)' : 'rgba(18,18,20,0.90)',
    headerBorder: isGlass ? 'rgba(255,255,255,0.5)'  : 'rgba(36,36,39,0.8)',
    accent:     isGlass ? '#F97316'         : '#EA580C',
    maroon:     '#431407',
    navBg:      isGlass ? 'rgba(255,255,255,0.82)' : 'rgba(24,24,28,1)',
    cardBg:     isGlass ? 'rgba(255,255,255,0.7)'  : '#1C1C1F',
    cardBorder: isGlass ? 'rgba(255,255,255,0.6)'  : '#242427',
    mutedText:  isGlass ? 'rgba(67,20,7,0.55)'     : '#6B7280',
  };
        ...
        >
          ...
          >
            ...
            <div className="hidden sm:flex items-center gap-2.5 pointer-events-auto">
              ...
              >
                {socialLinks.filter(l => l.url.trim()).map(link => {
              ...
              >
                <button
                  ...
                  style={{
                    background: isGlass ? '#fff' : T.cardBg,
                    color: isGlass ? T.accent : '#9CA3AF',
                    boxShadow: isGlass ? '0 2px 8px rgba(0,0,0,0.08)' : 'none',
                  }}
                  title={isGlass ? 'Switch to Dark' : 'Switch to Light'}
                >
                  {isGlass ? '☀️' : '🌙'}
                </button>
                ...
                {(['en', 'hi', 'ur'] as LangType[]).map(l => (
                  <button
                    ...
                    className="px-2 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all hover:scale-105"
                    style={{
                      background: language === l ? T.accent : 'transparent',
                      color: language === l ? '#fff' : T.mutedText,
                      boxShadow: language === l ? `0 2px 10px ${T.accent}55` : 'none',
                    }}
                  >
                    {l.toUpperCase()}
                  </button>
            ...
            <div className="flex sm:hidden items-center gap-2 pointer-events-auto">
              <button
                ...
                style={{ background: T.cardBg, borderColor: T.cardBorder, color: T.accent }}
              >
                {isGlass ? '☀️' : '🌙'}
              </button>
              <button
                ...
                <div className="flex gap-1.5">
                  {(['en', 'hi', 'ur'] as LangType[]).map(l => (
                    <button
                      ...
                      style={{
                        background: language === l ? T.accent : T.cardBg,
                        color: language === l ? '#fff' : T.mutedText,
                        border: `1px solid ${language === l ? T.accent : T.cardBorder}`,
                        boxShadow: language === l ? `0 2px 10px ${T.accent}55` : 'none',
                      }}
                    >
                      {l.toUpperCase()}
        ...
        <main className="max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex-1 flex flex-col gap-6 relative z-10">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              >
                {activeTab === 'portals' ? 'Academics & Portals' : 'Admin Secure Workstation'}
              </span>
            </div>
          ...
          <div
            key={contentKey}
            className="flex-1"
            style={{ animation: 'contentFadeUp 0.45s cubic-bezier(0.4,0,0.2,1) both' }}
          >
            {activeTab === 'portals' ? (
