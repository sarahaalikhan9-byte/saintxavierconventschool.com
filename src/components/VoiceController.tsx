...
interface VoiceControllerProps {
  ...
  setTheme: (theme: 'original' | 'glassNavy' | 'sunriseOrange') => void;
...
export default function VoiceController({
  ...
  const processVoiceCommand = (rawText: string) => {
    ...
    if (cmd.includes('hindi') || cmd.includes('हिन्दी') || cmd.includes('हिंदी') || cmd.includes('सेट हिंदी')) {
      setLanguage('hi');
      actionPerf = '🎯 Language updated to: हिन्दी';
      speakText('भाषा हिंदी में बदल दी गई है', 'hi');
    } else if (cmd.includes('urdu') || cmd.includes('उर्दू') || cmd.includes('اردو')) {
      setLanguage('ur');
      actionPerf = '🎯 Language updated to: اردو';
      speakText('زبان اردو میں تبدیل کر دی گئی ہے', 'ur');
    } else if (cmd.includes('english') || cmd.includes('अंग्रेजी') || cmd.includes('انگریزی')) {
      setLanguage('en');
    ...
    // Switch Tab Commands
    else if (cmd.includes('admin') || cmd.includes('desk') || cmd.includes('एडमिन') || cmd.includes('ایڈمن')) {
      setActiveTab('admin');
      actionPerf = '🚀 Switched to Admin Desk Command Center';
    } else if (cmd.includes('portal') || cmd.includes('student') || cmd.includes('पोर्टल') || cmd.includes('پورٹل')) {
      setActiveTab('portals');
      actionPerf = '🚀 Switched to Student Portals Deck';
    }

    // Switch Theme Commands
    else if (cmd.includes('dark') || cmd.includes('डार्क') || cmd.includes('काला')) {
      setTheme('original');
      actionPerf = '🎨 Theme switched to Dark Slate Mode';
    } else if (cmd.includes('glass') || cmd.includes('लाइट') || cmd.includes('गिलास')) {
      setTheme('glassNavy');
      actionPerf = '🎨 Theme switched to Glass Navy Mode';
    }

    // Scholar Register Filters
    else if (cmd.includes('nursery') || cmd.includes('eight') || cmd.includes('नर्सरी') || cmd.includes('پرائمری')) {
    ...
    } else if (cmd.includes('nine') || cmd.includes('twelve') || cmd.includes('ninth') || cmd.includes('नौवीं') || cmd.includes('نویں')) {
    ...
    } else if (cmd.includes('all') || cmd.includes('clear') || cmd.includes('सभी')) {
      localStorage.setItem('sxc_last_voice_filter', 'all');
      window.dispatchEvent(new CustomEvent('sxc_voice_filter_changed', { detail: 'all' }));
      actionPerf = '📂 Cleared filters: Displaying active scholar ledger!';
    }

    // Dispatch custom actions for ledger triggers
    else if (cmd.includes('pay') || cmd.includes('fee') || cmd.includes('outstanding') || cmd.includes('पैसे') || cmd.includes('فیس')) {
      window.dispatchEvent(new Event('sxc_voice_pay_fees'));
      actionPerf = '💳 Initiated Outstanding Fees Payment System!';
    } else {
      actionPerf = `Unrecognized command: "${rawText}". Try again!`;
    }
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        title="Voice Command & AI Assistant / आवाज निर्देश"
      >
        ...
        <>
          ...
          <div className={`absolute right-0 mt-2.5 w-80 p-4 rounded-2xl border shadow-2xl z-50 transform origin-top-right transition-all animate-fadeIn ${
            ...
            <div className="flex items-center gap-3 mb-3 bg-gray-500/5 p-2.5 rounded-xl border border-gray-500/5">
              ...
              <div className="text-left leading-none">
                <p className="text-[9px] font-black tracking-widest text-rose-500 uppercase">
                  {isListening ? '🎙️ MIC IS OPEN (TALK NOW)' : '🎙️ MIC OFFLINE (CLICK)'}
                </p>
                <div className="mt-1">
                  {transcript ? (
            ...
            <div className="flex flex-col gap-1 text-left border-t border-gray-500/10 pt-2">
              <span className="text-[9px] font-bold uppercase tracking-wider opacity-60">Suggestions / निर्देश:</span>
              <div className="grid grid-cols-2 gap-1 mt-1">
                {[
                  { label: 'Set Urdu 🇵🇰', cmd: 'set urdu' },
                  { label: 'Set Hindi 🇮🇳', cmd: 'set hindi' },
                  { label: 'Go to Admin ⚙️', cmd: 'go to admin' },
